...
#########################################################
#Set R directory and base directory for beach modeling
SNsys <- system("wmic bios get serialnumber", intern = TRUE, show.output.on.console = FALSE)
SN <- gsub("\\s","",SNsys)[2]
if(SN == "PB519R1") {Rlocal <- "D:/srcldata/R" #Laptop
                     baseDir <- "D:/srcldata/Beaches/GLRI beaches/Modeling/Models for 2014 season/"
}else {Rlocal <- "//igsarmewwssrc/SRCdata/R"
       baseDir <- "//igsarmewfsapa/projects/QW Monitoring Team/GLRI beaches/Modeling/Models for 2014 season/" #Network
}
#########################################################
...
# Set data and write directories
annualDataDir <- "Datasets 2013"
beachDir <- "Marina"
writeDir <- "Lasso"
dataDir <- paste(baseDir,annualDataDir,sep="")

beachAbbrev <- "MA"

#Set beach for naming files
beach  <- "BRMarina.test."
filetype <- c("MaxRowsTurb") #,"MaxRows","MaxCols","EnDDaT")
...
response <- c("log_beach_EColi")  #define the response variables


#  response <- "log_beach_EColi"

setwd(paste(dataDir,"/MaxRowsTurb/",sep=""))
...
setwd(paste(baseDir,"Model Exploration/",beachDir,"/",writeDir,sep=""))
...
for (i in 1:length(response)){
  ...
  for (l in 1:length(filetype)){
    pdf(paste(beach,filetype[l],"CVgraphsAlass.pdf",sep=""))
    CV.dev <- dev.cur()
  ...
  for (l in 1:length(filetype)){
    ...
    ############ Graphics ###########################
    y <- y.list[[l]]
    pdf(paste(beach,filetype[l],"ModelgraphsCenAndStepAlass.pdf",sep=""))
    model.dev <- dev.cur()
    ...
    write.csv(freq.vars.all,paste(beach,filetype[l],"ModelVarFreqCenAlass.csv",sep=""))
    write.csv(model.all,paste(beach,filetype[l],"ModelCICenAlass.csv",sep=""))
    write.csv(model.all.step,paste(beach,filetype[l],"ModelCIStepAlass.csv",sep=""))
    names(models) <- response
    names(models.step) <- response
    save(models,file=paste(beach,filetype[l],"ModelObjectsAlass.Rdata",sep=""))
    save(models.step,file=paste(beach,filetype[l],"ModelObjectsStepAlass.Rdata",sep=""))
    save(dfRegressionResults,file=paste(beach,filetype[l],"ObservationsAndPredictionsAlass.Rdata",sep=""))
    dev.off(model.dev)
  }
  write.csv(dfModelCoefsFinal,paste(beach,"ModelcoefficientsAlass",response[i],".csv",sep=""),row.names=F)
}
